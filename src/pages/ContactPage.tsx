...
const ContactPage = () => {
  ...
  const formRef = useScrollReveal({ once: true });
      ...
      <div className="relative overflow-hidden px-4 py-section sm:px-6 lg:px-8 section-3d">
        ...
        <div className="max-w-content mx-auto relative z-10">
          <div ref={headerRef.ref} className={`mx-auto mb-16 max-w-3xl text-center ${headerRef.isVisible ? 'active' : ''}`}>
            ...
            <h1 className={`mb-6 text-4xl font-bold leading-tight sm:text-5xl text-balance ${headerRef.isVisible ? 'reveal' : ''}`}>
              Get In Touch
            </h1>
          ...
          <div ref={formRef.ref} className={`grid lg:grid-cols-2 gap-12 ${formRef.isVisible ? 'active' : ''}`}>
            <div className={formRef.isVisible ? 'reveal-left' : ''}>
              <TiltCard
                maxTilt={6}
                className="card-3d card-3d-shine overflow-hidden rounded-lg border border-border bg-card shadow-sm transition-all duration-300 hover:shadow-xl"
              >
                ...
                <CardContent>
                  <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <Button type="submit" className="group w-full transition-all duration-300 hover:scale-[1.02]">
                      Send Message
            ...
            <div className={formRef.isVisible ? 'reveal-right' : ''}>
              <div className="space-y-6">
                <div className="card-3d card-3d-shine rounded-lg border border-border bg-card shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
                  <CardHeader>
                  ...
                  <CardContent className="space-y-6">
                    {contactItems.map((item) => {
                      ...
                      return (
                        <div
                          key={item.label}
                          className="flex items-center gap-4 rounded-lg border border-border bg-background p-4 transition-all duration-300 hover:border-primary/30 hover:shadow-md"
                        >
                  ...
                  <CardContent>
                    <div className="flex flex-wrap gap-3">
                      {socialLinks.map((link) => {
                        ...
                        return (
                          <Button key={link.label} variant="outline" size="sm" asChild className="transition-all duration-300 hover:scale-105">
                            <a
                              href={link.href}
                              target="_blank"
